...
def api_status():
    ...
    livros_map = {l["id"]: l for l in livros}
    for e in atrasados:
        dias_atraso = (date.today() - date.fromisoformat(e["data_prevista_devolucao"])).days
        livro_nome = livros_map.get(e.get("livro_id"), {}).get("nome", "— livro removido —")
        notificar_se_ausente(
            modulo="biblioteca", tipo="emprestimo_atrasado", referencia_id=e["id"],
            titulo=f"Devolução atrasada — {e.get('aluno_nome','')}",
            mensagem=f"{livro_nome} · {dias_atraso} dia(s) de atraso",
            url="/biblioteca/", destinatario_tipo="perfil", destinatario_valor="bibliotecaria",
        )
...
@login_obrigatorio
def api_turmas():
    """Lista fixa de turmas, para o formulário de cadastro rápido de aluno."""
...
def api_buscar_alunos():
    """
    Busca no cadastro único de alunos (compartilhado com Ocorrências e
    Prevenção de Evasão). Query: busca (nome parcial, para autocomplete).
    """
    busca = request.args.get("busca", "").strip()
    db = _get_db()
    resultados = cadastro_alunos.buscar(busca=busca, db=db, limite=20 if busca else None)
...
def api_cadastrar_aluno():
    """
    Cadastra um novo aluno diretamente pela Biblioteca. O aluno criado
    aqui aparece automaticamente também em Ocorrências e Prevenção de
    Evasão — é o mesmo cadastro único usado em todo o sistema.
    Body: nome, turma.
    """
    dados = request.get_json(force=True)
    db = _get_db()
    try:
        registro = cadastro_alunos.criar(dados.get("nome", ""), dados.get("turma", ""), db=db)
...
def api_novo_emprestimo():
    """
    Body JSON:
      livro_id, aluno_id, dias (opcional, padrão 7)

    O aluno precisa já existir no cadastro único (buscado ou cadastrado
    via /api/alunos). Nome, turma e curso são gravados automaticamente
    a partir do cadastro no momento do empréstimo — um "retrato" que
    preserva o histórico mesmo se o aluno mudar de turma depois.
    """
    dados    = request.get_json(force=True)
    ...
    if not livro_id or not aluno_id:
        return jsonify({"ok": False, "erro": "Selecione o livro e o aluno."}), 400
    ...
    if not aluno:
        return jsonify({"ok": False, "erro": "Aluno não encontrado no cadastro."}), 404
...
def api_devolver(emprestimo_id: int):
    ...
    resolver_notificacao(modulo="biblioteca", tipo="emprestimo_atrasado", referencia_id=emprestimo_id)
    return jsonify({"ok": True})
