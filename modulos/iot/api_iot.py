...
def _carregar_estado_portoes() -> dict:
    if ESTADO_PORTOES_PATH.exists():
        try:
            ...
            logging.warning(f"[IOT] Estado de portões corrompido/ilegível, usando padrão: {e}")
...
def _carregar_estado_ac() -> dict:
    if ESTADO_AC_PATH.exists():
        try:
            ...
            logging.warning(f"[IOT] Estado de ar-condicionado corrompido/ilegível, usando padrão: {e}")
...
def _loop_automacoes():
    ...
    while True:
        try:
            ...
            # "portoes" e "ar_condicionado" só existem no config se a
            # escola realmente ativou esses dispositivos (ver
            # wizard/logica.py:montar_dispositivos_iot). Sem esse check,
            # uma instalação que só usa sensores de água, por exemplo,
            # tem essa exceção disparada e logada a cada 20s, para sempre.
            if "portoes" in cfg:
                with _lock_portoes:
