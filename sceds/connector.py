...
class SCEDS:
    ...
    def inserir_com_validacao(self, tabela: str, dados: dict, validador) -> dict:
        """
        Como inserir(), mas roda validador(registros_existentes) de forma
        atômica junto com a escrita (evita corrida entre checagem e
        inserção quando a regra depende de todos os registros da
        tabela, não só de um campo único). Ver SCEDSEngine.inserir_com_validacao.
        """
        return self._engine.inserir_com_validacao(tabela, dados, validador)
    ...
    def atualizar_com_validacao(self, tabela: str, novos_dados: dict, onde: dict, validador) -> int:
        """
        Como atualizar(), mas roda validador(registros_existentes) de
        forma atômica junto com a escrita. Ver SCEDSEngine.atualizar_com_validacao.
        """
        return self._engine.atualizar_com_validacao(tabela, novos_dados, onde, validador)
    ...
    def adicionar_coluna_se_ausente(self, tabela: str, coluna: dict) -> bool:
        """Adiciona uma coluna ao schema de uma tabela existente, se ainda não houver. Retorna True se adicionou."""
        return self._engine.adicionar_coluna_se_ausente(tabela, coluna)
